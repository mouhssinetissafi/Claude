...
yuv420p with scale+crop (never stretched) and audio removed. Normalization is
skipped when an up-to-date output already exists (content hash sidecar).
"""
...
def normalize_clips(job: DiscoveredJob, paths: JobPaths, cfg: Config, *, force: bool = False) -> list[SourceClip]:
    """Probe, validate and normalize every clip; write media_manifest.json."""
    ...
    for index, clip in enumerate(job.clips, start=1):
        try:
            ...
            results.append(SourceClip(file=clip.name, path=str(clip), content_hash="", status="rejected", reason=f"unreadable: {exc}"))
    ...
    log.info("Normalization: %d usable, %d rejected", len(usable), len(results) - len(usable))
